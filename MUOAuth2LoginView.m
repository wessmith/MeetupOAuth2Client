             ...
             delegate:(id <MUOAuth2LoginViewDelegate>)delegate;
{
    ...
    if (self) {
        ...
        UIEdgeInsets contentInsets = UIEdgeInsetsMake(kPadding + statusBarheight, kPadding, kPadding, kPadding);
        self.frame = UIEdgeInsetsInsetRect(window.bounds, contentInsets);
        [window addSubview:self];
        
        // Add an off-white background view.
        UIView *backgroundView = [[UIView alloc] initWithFrame:CGRectInset(self.bounds, kPadding, kPadding)];
...
{
    if (!_closeButton) {
        _closeButton = [[MUCloseButton alloc] initWithFrame:CGRectMake(self.frame.size.width, -14, 44, 44)];
...
- (void)show
{
    ...
    [UIView animateWithDuration:kShowAnimationDuration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        [UIView animateWithDuration:kBounceAnimationDuration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            [UIView animateWithDuration:kBounceAnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
...
{
    NSLog(@"Started loading authorization view...");
}

////////////////////////////////////////////////////////////////////////////////
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    NSLog(@"Finished loading authorization view.");
    
    [self.activityView stopAnimating];
    
    // Adjust the width of the webview content to fit nicely in the slightly smaller frame.
    NSString *javascript = [NSString stringWithFormat:@"document.body.style.width='%dpx'", (int)self.webView.bounds.size.width];
